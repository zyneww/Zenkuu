...
import { getRanking, type AssetClass, type MarketAsset } from '@zenith/data'
              ...
              return (
                <li key={item.id}>
                  <Link
                    ...
                    className="group flex items-center gap-3 px-4 py-3 transition-colors duration-150 hover:bg-surface-muted"
                  >
                    {quote ? (
                      <AssetLogo asset={quote} size={24} />
                    ) : (
                      <span
                        className="h-6 w-6 shrink-0 bg-surface-muted"
                        aria-hidden="true"
                      />
                    )}
    ...
    <div className="mx-auto max-w-3xl space-y-10 py-6">
      <header className="space-y-2">
        ...
        <p className="text-sm leading-relaxed text-ink-muted">
          Vos actifs suivis et vos préférences d’affichage. ZENITH n’exécute aucun ordre
